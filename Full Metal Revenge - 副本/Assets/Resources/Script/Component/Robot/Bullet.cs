namespace MoenenGames.VoxelRobot
{
    ...
    public class Bullet : MonoBehaviour
    {
        ...
        void Colliding(Transform tf)
        {
            ...
            if (Shooter)
            {
                if (Shooter == tf || Shooter.tag == tf.tag)
                {
                    return;
                }
            }
            ...
            if (!DontDestroyOnHit)
            {
                ...
                if(Shooter)
                {
                    if (tf.gameObject.layer == LayerMask.NameToLayer("Robot"))
                    {
                        tf.GetComponent<RobotInfo>().GetDamage(Damage);
                    }
                    if (tf.gameObject.layer == LayerMask.NameToLayer("Building"))
                    {
                        tf.GetComponent<ResourcesBuilding>().Neutral_GetDamage(Shooter, Damage);
                    }
                    if (ExplosionRadius > 0.5f)
                    {
                        Collider[] target = Physics.OverlapSphere(transform.position, ExplosionRadius);


                        for (int i = 0; i < target.Length; i++)
                        {
                            if (Shooter.tag != target[i].transform.tag && Shooter.gameObject.layer == target[i].gameObject.layer)
                            {
                                target[i].transform.GetComponent<RobotInfo>().GetDamage(Damage);
                            }
